...
import { formatDuration, splitDurationMinutes, combineDurationMinutes } from "@/lib/duration";
...
export default function AppointmentsPage() {
  ...
  function openEditModal(appt: AppointmentItem) {
    ...
    const { hours, minutes } = splitDurationMinutes(appt.durationMinutes || 60);
  ...
  async function handleSaveEdit(e: React.FormEvent) {
    ...
    setEditError(null);
    const totalMinutes = combineDurationMinutes(editForm.durationHours, editForm.durationMinutes);
    if (totalMinutes <= 0) {
      setEditError("Appointment duration must be greater than 0 minutes.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        setEditError(err.error || "Failed to update appointment");
      }
    } catch (err) {
      console.error(err);
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="flex items-center gap-1.5 text-xs font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-lg">
                        ...
                        <span className="text-slate-400 font-normal">({formatDuration(appt.durationMinutes)})</span>
                      </span>
                    </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[11px] font-medium text-slate-500">
                      {combineDurationMinutes(editForm.durationHours, editForm.durationMinutes) > 0 ? (
                        <span className="text-blue-600 font-semibold">{formatDuration(combineDurationMinutes(editForm.durationHours, editForm.durationMinutes))}</span>
                      ) : (
                        <span className="text-rose-500 font-semibold">Must be &gt; 0m</span>
                      )}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => setEditForm({ ...editForm, durationHours: Math.max(0, parseInt(e.target.value) || 0) })}
                        className="w-full text-sm pl-3 pr-8 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                      <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2.5 text-xs text-slate-400 font-medium">
                        hrs
                      </span>
                    </div>
                    <div>
